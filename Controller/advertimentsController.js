...
const addAdvertisement = async (req, res) => {
  try {
    ...
    const images = req.files.map((file) => ({
      ...
      url: `${process.env.PORT_URI}/advertisement/${file.filename}`, // Adjust the URL based on your server configuration
    }));
...
const getBookingHistory = async (req, res) => {
  try {
    ...
    // Find the user by ID
    const user = await User.findById(userId).populate("bookHistory");
